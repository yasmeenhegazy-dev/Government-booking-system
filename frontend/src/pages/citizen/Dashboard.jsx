  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="إجمالي الحجوزات" value={stats.total} icon={Calendar} border="border-navy-300" />
        <StatCard label="القادمة" value={stats.upcoming} icon={Hourglass} border="border-gold-300" />
        <StatCard label="المنجزة" value={stats.completed} icon={CheckCircle2} border="border-green-300" />
        <StatCard label="الملغاة" value={stats.cancelled} icon={XCircle} border="border-red-300" />
      </div>
      ...
      <section className="card-standard">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">أقرب {upcomingList.length} مواعيد</p>
          </div>
          <Link
            to="/citizen/appointments"
              ...
              <tbody>
                {upcomingList.map((apt) => (
                  <tr key={apt._id} className="border-b border-slate-100 last:border-0">
                    <td className="py-3 font-medium text-navy-600">{apt.serviceId?.name || "—"}</td>
                    <td className="py-3 text-slate-600 hidden sm:table-cell">{apt.branchId?.name || "—"}</td>
                    <td className="py-3 text-slate-600">{formatDate(apt.slotId?.date)}</td>
                    <td className="py-3 text-slate-600" dir="ltr">{apt.slotId?.startTime}</td>
