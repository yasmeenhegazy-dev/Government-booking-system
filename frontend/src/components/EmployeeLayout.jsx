...
const NAV_ITEMS = [
  ...
  { to: "/employee/scan", label: "مسح QR", icon: ScanLine },
  { to: "/employee/appointments", label: "المراجعة اليومية", icon: ClipboardList },
  { to: "/employee/profile", label: "الملف الشخصي", icon: UserCog },
];

export default function EmployeeLayout() {
  const location = useLocation();
  const [mobileOpen, setMobileOpen] = useState(false);
  const { session: employee, logout } = useEmployeeSession();
