...
import { useEmployeeSession } from "../../lib/employeeAuth";
...
export default function EmployeeDashboard() {
  ...
  const todayStr = useMemo(() => localDayString(new Date()), []);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError(null);
        const result = await getTodayAppointments(session.branch._id, todayStr);
  ...
  if (sessionLoading || (!session && loading)) {
    return (
      <EmployeeLayout employee={null} onLogout={logout}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="card-standard lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-lg font-bold text-navy-500">الحجوزات القادمة اليوم</h2>
                <p className="text-xs text-slate-500 mt-0.5">{upcoming.length} حجز قادم</p>
              </div>
              <Link
            ...
            {loading ? (
              ...
              <div className="text-center py-10 text-slate-500">
                ...
                <p className="text-sm">لا توجد حجوزات قادمة لهذا اليوم</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-xs text-slate-500 border-b border-slate-200">
                      ...
                      <th className="text-start font-medium py-3 hidden sm:table-cell">الخدمة</th>
