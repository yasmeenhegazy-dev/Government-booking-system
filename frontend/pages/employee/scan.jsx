    ...
    async function startCamera() {
      ...
      try {
        ...
        await inst.start(
          ...
          async (decodedText) => {
            // Stop immediately so we don't get repeat callbacks while verifying
            try {
              await inst.stop();
              inst._isScanning = false;
            } catch {}
        ...
        setResult({
          ...
          message:
            err?.message?.includes("Permission")
              ? "تم رفض الوصول للكاميرا. اسمح بالوصول من إعدادات المتصفح أو استخدم رفع صورة."
              : "تعذر تشغيل الكاميرا. تأكد من توصيلها وحاول مرة أخرى.",
        });
      } finally {
        setScanning(false);
      }
  ...
  if (sessionLoading) {
    return (
      <EmployeeLayout employee={null} onLogout={logout}>
        <div className="text-center text-slate-500 py-20">جاري التحميل...</div>
      </EmployeeLayout>
    );
  }
